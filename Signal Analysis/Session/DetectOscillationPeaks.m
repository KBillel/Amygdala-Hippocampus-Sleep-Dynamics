...
peaks.time = filtered_lfp(peaks.loc,1);
troughs.time = filtered_lfp(troughs.loc,1);
% 
...
for i = 1:size(peaks.loc)-1
    ...
    t_next_troughs = troughs.time(troughs.time > t_peak);
...
for i = 1:size(troughs.loc)-1
    ...
    t_next_peaks = peaks.time(t_trough < peaks.time);
