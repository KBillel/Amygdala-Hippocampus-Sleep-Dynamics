...
nALL = max(spks.all(:,4));
metadata = [repmat(rat,nALL,1) repmat(jour,nALL,1) unique(spks.all(:,2:4),'rows')];
...
if strcmpi(str_name,'all')
    ...
    if strcmpi(savevar,'on')
        ...
        save(['CorrMatrixAll_' num2str(binSize) '.mat'],'corrMatrix','metadata')
        disp(['CorrMatrixAll_' num2str(binSize) '.mat SAVED'])
    end
